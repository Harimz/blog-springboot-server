package com.spring.blog.upload.service.impl;
...
public class UploadServiceImpl implements UploadService {
    ...
    @Override
    public PresignUploadResponse presignUserAvatar(UUID userId, PresignUploadRequest req) {
        if (req.getContentType() == null || !req.getContentType().startsWith("image/")) {
            throw new IllegalArgumentException("Only image uploads are allowed");
        }

        String ext = getExtension(req.getFilename());
        String key = "users/" + userId + "/avatar/" + UUID.randomUUID() + (ext.isBlank() ? "" : "." + ext);
